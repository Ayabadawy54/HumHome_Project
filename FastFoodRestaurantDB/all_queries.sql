...
-----------------------------------------------------------------------------------------
-- triggers
-- add column totalQuantity 
--and make a trigger to make an example for consistant data 
ALTER TABLE Categories
ADD  TotalQuantity int ;
...
-- when insert new item in products --> the total quantity will change 

create trigger tgTotalQuantityInsert
on products 
after insert
as
begin
declare @new_quantity int 
select @new_quantity=i.Quantity from inserted as i
declare @category_id int 
select @category_id = i.CategoryId from inserted as i
update Categories 
set TotalQuantity=coalesce( TotalQuantity ,0)+@new_quantity
...
--insert quantity -->200 categoryid --> 1
INSERT INTO Products (Name, Description, Price, Quantity, CategoryId, CreatedDate)
VALUES 
('Classic chicken Burger', 'A juicy beef patty with lettuce, tomato, onion, pickles, ketchup, and mustard on a sesame seed bun.', 100.00, 200, 1, GETDATE())
--try after
select * from Categories
------------------------------------------------------
--when delete
create trigger tgTotalQuantityDelete
on products 
after delete
as
begin
declare @old_quantity int 
select @old_quantity=d.Quantity from deleted as d
declare @category_id int 
select @category_id = d.CategoryId from deleted as d
update Categories 
set TotalQuantity=coalesce( TotalQuantity ,0)- @old_quantity
where CategoryId=@category_id 

end 

delete from Products where name='Classic chicken Burger'
----------------------------------------------------
--when update 
create trigger tgTotalQuantityUpdate
on products 
after update
as
begin
...
select @category_id = d.CategoryId from deleted as d
update Categories 
set TotalQuantity=coalesce( TotalQuantity ,0)- @old_quantity +@new_quantity
where CategoryId=@category_id 
